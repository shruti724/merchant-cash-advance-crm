# MCA Portfolio CRM — Backend

A production-style, multi-tenant SaaS CRM backend demonstrating **Clean
Architecture**, **Domain-Driven Design**, **RBAC**, **row-level
multi-tenancy**, and an **event-driven workflow engine** — built with
**NestJS**, **Prisma**, and **PostgreSQL**.

> Part of the [mca-portfolio-crm](../README.md) monorepo. This is a
> simplified, portfolio edition of a real multi-tenant MCA (Merchant Cash
> Advance) origination platform; proprietary underwriting/commission logic
> was removed and replaced with a generic Lead → Application workflow. See
...
From the **repo root**, `docker compose up --build` runs Postgres, this
API, and the frontend together (see [top-level README](../README.md)).
...
any use case writing to it directly. The [frontend](../frontend) gives
you the same flow as a UI instead of curl.
